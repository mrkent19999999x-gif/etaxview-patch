package seatechit.ihtkk.tool.hquanformnew;
...
/* loaded from: itaxviewer.jar:seatechit/ihtkk/tool/hquanformnew/HquanFormNew.class */
public class HquanFormNew extends HSoThue {
    public HquanFormNew(Document tkhaiDoc, String tkhaiFileName, ConfigInfo config) throws IOException, ParserConfigurationException, ITaxViewerException {
        super(tkhaiDoc, tkhaiFileName, config);
        String nodeName;
        String nodeName2;
        DMucHquanNew dmucHquan = config.getDmHquanNew();
        XPathFactory xpathFactory = XPathFactory.newInstance();
        XPath xpath = xpathFactory.newXPath();
        this.maHSo = null;
        try {
            XPathExpression expr = xpath.compile("*[local-name()='Envelope']/*[local-name()='Body']/*[local-name()='ListExImDeclarationResponse']/*[local-name()='Declarations']/*[local-name()='ExImCode']");
            this.maHSo = (String) expr.evaluate(tkhaiDoc, XPathConstants.STRING);
        } catch (XPathExpressionException e) {
            this.maHSo = null;
            e.printStackTrace();
        }
        if (this.maHSo == null || "".equals(this.maHSo)) {
            try {
                XPathExpression expr2 = xpath.compile("*[local-name()='Envelope']/*[local-name()='Body']/*[local-name()='InquiryAMADeclarationResponse']/*[local-name()='AMADeclaration']/*[local-name()='ExImCode']");
                this.maHSo = (String) expr2.evaluate(tkhaiDoc, XPathConstants.STRING);
                this.maHSo = "BSSTQ";
    ...
    private Collection getPLucList(DMucHquanNew dmTBao) {
        String plucID;
