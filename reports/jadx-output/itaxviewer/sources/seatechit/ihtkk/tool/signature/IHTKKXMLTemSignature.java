...
/* loaded from: itaxviewer.jar:seatechit/ihtkk/tool/signature/IHTKKXMLTemSignature.class */
public class IHTKKXMLTemSignature {
    private X509Certificate[] rootCerts;
    private X509Certificate[] trustedCerts;

    public IHTKKXMLTemSignature(X509Certificate[] rootCerts, X509Certificate[] trustedCerts) {
                ...
                try {
                    try {
                        ...
                        if (name1 != null) {
                            ...
                            String tgianKy = TaxViewerUtils.ConvertDateStringToFormat(sgtime.getFirstChild().getTextContent(), "ddMMyyyyHHmmss");
                ...
                try {
                    try {
                        ...
                        if (name1 != null) {
                            ...
                            Element sgtime = (Element) ndlst.item(0);
                            String ma_tkhai = doc.getElementsByTagName("maTKhai").item(0).getTextContent();
                            if (isHDDT(ma_tkhai)) {
    ...
    public Collection verifyXMLSignature(Document doc, String xPath_data) {
        new CertVerifier(this.rootCerts, this.trustedCerts);
        ArrayList resultArr = new ArrayList();
        XMLSignatureFactory fac = XMLSignatureFactory.getInstance("DOM");
        NodeList signl = doc.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "Signature");
        Node nodeToSign = null;
