...
/* loaded from: itaxviewer.jar:seatechit/ihtkk/tool/taxreturn/BLaiInvoice.class */
public class BLaiInvoice extends HSoThue {
    public BLaiInvoice(Document tkhaiDoc, String tkhaiFileName, ConfigInfo config) throws IOException, ParserConfigurationException, ITaxViewerException {
        ...
        if (this.prefix != null) {
            nodeName = String.valueOf(this.prefix) + ":KHMCTu";
        } else {
            nodeName = "KHMCTu";
        }
        Node node = tkhaiDoc.getElementsByTagName(nodeName).item(0);
        if (node == null) {
            throw new ITaxViewerException("Cấu trúc tệp hồ sơ không đúng: không có thẻ dữ liệu 'KHMCTu'");
        }
        ...
        tkhaiDoc.getElementsByTagName(nodeName3).item(0);
