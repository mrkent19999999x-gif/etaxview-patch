...
/* loaded from: itaxviewer.jar:seatechit/ihtkk/tool/taxRegistration/DKyThue.class */
public class DKyThue extends HSoThue {
    public DKyThue(Document tkhaiDoc, String tkhaiFileName, ConfigInfo config) throws IOException, ParserConfigurationException, ITaxViewerException {
        super(tkhaiDoc, tkhaiFileName, config);
        String nodeName;
        String nodeName2;
        DMucDKy dmDKy = config.getDmDKy();
        if (this.prefix != null) {
            nodeName = String.valueOf(this.prefix) + ":maDKy";
        } else {
            nodeName = "maDKy";
        }
        ...
        if (this.prefix != null) {
            nodeName2 = String.valueOf(this.prefix) + ":pBanDKy";
        } else {
            nodeName2 = "pBanDKy";
        }
        ...
        this.xsdFile = dmDKy.getXSDTBao(this.maHSo, this.pbanHSoXML);
        if (this.xsdFile == null) {
            throw new ITaxViewerException("Mã đăng ký (" + this.maHSo + ") hoặc phiên bản đăng ký (" + this.pbanHSoXML + ") không đúng");
        }
        this.viewMethod = dmDKy.getTBaoViewType(this.maHSo, this.pbanHSoXML);
        this.excelFile = dmDKy.getExcelTemplateTTBao(this.maHSo, this.pbanHSoXML);
        this.xsltFile = dmDKy.getXSLTTBao(this.maHSo, this.pbanHSoXML);
        this.orientation = dmDKy.getTKhaiOrientation(this.maHSo, this.pbanHSoXML);
        this.plucList = getPLucList(dmDKy);
        this.sigValidationResult = new IHTKKXMLSignature(config.getRootCerts(), config.getTrustedCerts()).verifyXMLSignature(tkhaiDoc, config.getDkyXpath());
    ...
    private String createTemFileForView() throws ITaxViewerException, IOException {
        ...
        String content2 = content.replaceAll("xmlns=\"http://kekhaithue.gdt.gov.vn/HSoDKy\"", "");
    ...
    private Collection getPLucList(DMucDKy dmTBao) {
        String plucID;
