	...
	<xsl:template match="/">
		...
		<br/>
				...
				<xsl:if test="$ndungTBao/trangThai='N'">
						...
						mã giao dịch điện tử <xsl:value-of select="$ndungTBao/maGiaoDichDTu"/>. Cơ quan thuế thông báo về việc không chấp nhận hồ sơ khai thuế/BCTC/BCAC của <xsl:value-of select="$NNhanTBaoThue/tenNNhan"/> 
						<xsl:if test="$ttinDaiLyThue/tenDLT !=''">
						do <xsl:value-of select="$ttinDaiLyThue/tenDLT"/> 
						</xsl:if>
								...
								<tr>
								 ...
								 <xsl:for-each select="$ndungTBao/IDKhoanNop/CTietIDKhoanNop">
											...
											<tr>
											  ...
											  <td class="align-c">
											    <xsl:if test="$ndungTBao/HoSoThue/CTietHoSoThue/loaiToKhai ='Bổ sung'">
