 ...
 <xsl:include href="../../include/TKhaiFooter.xsl"/>    
  <xsl:include href="../../common/common.xsl"/> 
	...
	<xsl:variable name="tkchinh" select='HSoThueDTu/HSoKhaiThue/CTieuTKhaiChinh' />
        ...
        <xsl:variable name="ghiChuPL" select="'(Áp dụng đối với doanh nghiệp khai thác tàu)'"/>
  ...
  <xsl:if test="HSoThueDTu/HSoKhaiThue/PLuc/PL_01_1_VTNN">	
	...
	<xsl:call-template name="tkhaiHeaderPL01-1_VTNN_TT80">
		<xsl:with-param name="mauTKhai"   select="'01-1/VTNN'"/>
		<xsl:with-param name="moTaBieuMau"   select="$moTaBieuMau"/>
		<xsl:with-param name="tenPL"   select="'BẢNG KÊ THU NHẬP VẬN TẢI QUỐC TẾ'"/>
		<xsl:with-param name="ghiChuPL"   select="'(Áp dụng đối với doanh nghiệp khai thác tàu)'"/>
			<xsl:with-param name="ghiChuPL1"   select="'(Kèm theo Tờ khai quyết toán thuế TNDN đối với hãng vận tải nước ngoài mẫu: 01/VTNN)'"/>
	  </xsl:call-template>
	  
	<xsl:for-each select="$pluc01/NoiDungPL01_1_VTNN">
				...
				<div class="content">
					...
					<table class="tkhai_table">
                        <tr>
							...
							<td class="align-c" colspan="2"><b>Thông tin khách hàng</b></td>
		...
		<tr>
			<td>	
				<xsl:call-template name="tkhaiFooter"/>
					<div class="ghichu">	  
						<u><b>Ghi chú:</b></u><br/>
						- <i>Cột (1) đến (6) ghi trên cơ sở vận đơn xuất cho khách hàng</i><br/>
						- <i>Cột (7) ghi doanh thu tính thuế của người trực tiếp điều hành phương tiện vận tải không bao gồm tiền thuế đã trả cho doanh nghiệp khác tham gia vận tải từ cảng Việt Nam đến cảng trung gian.</i><br/>
					</div>
					<div id="sigDiv"></div>
			</td>
		</tr>
