...
<xsl:include href="../../include/TKhaiHeader.xsl"/>
 <xsl:include href="../../include/TKhaiFooter.xsl"/>    
  <xsl:include href="../../common/common.xsl"/>
	...
	<xsl:variable name="ttchung" select='HSoThueDTu/HSoKhaiThue/TTinChung/TTinTKhaiThue/TKhaiThue/GiaHan' />
...
<div class="align-l">
				...
				<xsl:if test="not(HSoThueDTu/HSoKhaiThue/TTinChung/TTinTKhaiThue/TKhaiThue/GiaHan)">[] Gia hạn</xsl:if>	
					<xsl:if test="$ttchung/maLyDoGiaHan='01'"> [x] Gia hạn</xsl:if>
					<xsl:if test="$ttchung/maLyDoGiaHan = '02'">[x] Gia hạn</xsl:if>
					<xsl:if test="$ttchung/maLyDoGiaHan = '03'"> [x] Gia hạn</xsl:if>
					<xsl:if test="$ttchung/maLyDoGiaHan = '99'">[x] Gia hạn</xsl:if>
						 </div>
	     <xsl:if test="(HSoThueDTu/HSoKhaiThue/TTinChung/TTinTKhaiThue/TKhaiThue/GiaHan)">     
	     <xsl:if test="HSoThueDTu/HSoKhaiThue/TTinChung/TTinTKhaiThue/TKhaiThue/GiaHan != ''">		 
				<div>Trường hợp được gia hạn: <xsl:value-of select="$ttchung/lyDoGiaHan"/>	</div>
				</xsl:if>	
			</xsl:if>
		<div class="donvitien"><i>Đơn vị tiền: đồng Việt Nam</i></div>
		 <table class="tkhai_table">
			 ...
			 <tr>
				...
				<td class="align-c" colspan="2"><xsl:if test="$tkchinh/ct21='true'"> [x] </xsl:if>  <xsl:if test="$tkchinh/ct21 = 'false'">[]</xsl:if>			 <xsl:if test="$tkchinh/ct21 = '1'"> [x]</xsl:if> <xsl:if test="$tkchinh/ct21 = '0'">[]</xsl:if>			</td>
			 </tr>
			 <tr>
