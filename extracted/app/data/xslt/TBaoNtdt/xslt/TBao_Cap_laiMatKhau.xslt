		...
		<div>
			<table>
				<tbody>
					...
					<tr>
						<td/>
						<td>Mã số thuế:<xsl:value-of select="$NNhanTBaoThue/maNNhan"/>
						</td>
					</tr>
				</tbody>
			</table>
			<br/>
			<xsl:value-of select="ihtkk:timeToString($ndungTBao/gioPhutGiayXacNhan,'hh')"/> giờ <xsl:value-of select="ihtkk:timeToString($ndungTBao/gioPhutGiayXacNhan,'mm')"/> phút <xsl:value-of select="ihtkk:timeToString($ndungTBao/gioPhutGiayXacNhan,'ss')"/> giây
			, ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'yyyy')"/>, đơn vị đã được Cơ quan Thuế cập nhật thay đổi thông tin tài khoản khai thuế điện tử, gồm có:<br/>
			- Địa chỉ thư điện tử: <xsl:value-of select="$ndungTBao/email"/>
			<br/>
			- Điện thoại: <xsl:value-of select="$ndungTBao/dienThoai"/>
			<br/>
			- Người liên hệ:  <xsl:value-of select="$ndungTBao/nguoiLienHe"/>
			<br/>
			- Số chứng thư số:  <xsl:value-of select="$ndungTBao/serial"/>
			<br/>
			- Tên tổ chức chứng thực chữ ký số công cộng:  <xsl:value-of select="$ndungTBao/issuer"/>
			<br/>
