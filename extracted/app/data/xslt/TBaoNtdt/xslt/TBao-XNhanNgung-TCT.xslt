...
<xsl:include href="../common/common.xsl"/>
	<xsl:template match="/">
		...
		<xsl:call-template name="tbaoHeader">
			...
			<xsl:with-param name="motaTBao"   select="'V/v Xác nhận ngừng sử dụng dịch vụ nộp thuế điện tử qua NHTM'" />
		</xsl:call-template>
		...
		<div>
			...
			, ngày <xsl:value-of select="ihtkk:stringDatetime($tbThue/NGAY_TBAO,'dd')"/>/<xsl:value-of select="ihtkk:stringDatetime($tbThue/NGAY_TBAO,'mm')"/>/<xsl:value-of select="ihtkk:stringDatetime($tbThue/NGAY_TBAO,'yyyy')"/>, Cơ quan thuế xác nhận NNT đã ngừng sử dụng dịch vụ nộp thuế điện tử.
			<br/>
			<br/>
			Cơ quan thuế thông báo để NNT được biết./.<br/>
