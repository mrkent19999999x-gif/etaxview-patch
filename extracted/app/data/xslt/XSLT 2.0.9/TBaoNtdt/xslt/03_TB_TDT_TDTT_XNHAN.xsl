  ...
  <xsl:template match="/">
   <style type="text/css">ol { margin: 0; padding: 0 }
      ...
      .tb_strong { align: center; }</style>
       ...
       <xsl:for-each select="TBAO_TKHOAN_NT">
        <xsl:variable name="TBAO_TKHOAN_NT" select="." />
        <xsl:for-each select="$TBAO_TKHOAN_NT">
		  <xsl:variable name="tthaixachnhan" select="TTHAI_XNHAN" />
		   <xsl:variable name="vanid" select="VAN_ID" />
		...
		<tbody>
			<tr>
				<td style="vertical-align: top;"   align="right"   colspan="2">
					...
					Mẫu số: <b><xsl:value-of select="MAU_TBAO"/>					
					</b><br/>
            ...
            <p  style="margin-left: 62px; font-size:15px; "> <span  style=" font-size:15px; font-style:italic;">Mã số thuế của NNT: &#160;  </span>
			  ...
			  <xsl:choose>
        <xsl:when test="$vanid='00000' ">
            <!-- code for dates later than 2014-10-01  -->
			 <p  style="margin-left: 62px;line-height: 1.5; margin-top: 2pt; direction: ltr; padding-bottom: 0pt; font-size:15px;">Căn cứ hồ sơ đăng ký thay đổi thuế điện tử qua Cổng thông tin điện tử của Tổng cục Thuế của người nộp thuế.     
            </p>
			
			
        </xsl:when>
        <xsl:otherwise>
		        <p  style="margin-left: 62px;line-height: 1.5; margin-top: 2pt; direction: ltr; padding-bottom: 0pt; font-size:15px;">Căn cứ hồ sơ đăng ký thay đổi thuế điện tử qua tổ chức cung cấp dịch vụ T-VAN của người nộp thuế.     
            </p>
        ...
        <xsl:when test="$tthaixachnhan='Y' ">
		   <p  style="margin-left: 62px; line-height: 1.5; margin-top: 1pt; direction: ltr; padding-bottom: 0pt; font-size:15px;">Cơ quan thuế chấp nhận đề nghị về việc đăng ký thay đổi 

  <xsl:choose>
     <xsl:when test="$vanid='00000' ">
            <!-- code for dates later than 2014-10-01  -->
			qua Cổng thông tin điện tử của Tổng cục Thuế của người nộp thuế kể từ&#160;
        </xsl:when>
        <xsl:otherwise>
		qua tổ chức cung cấp dịch vụ T-VAN của người nộp thuế kể từ&#160;
            <!-- code for dates earlier than or equal to 2014-10-01 -->
        </xsl:otherwise>
    </xsl:choose>
        ...
        <xsl:otherwise>
		  <p  style="margin-left: 62px; line-height: 1.5; margin-top: 1pt; direction: ltr; padding-bottom: 0pt; font-size:15px;">Cơ quan thuế không chấp nhận đề nghị về việc đăng ký thay đổi của người nộp thuế. &#160;
        ...
        <xsl:when test="$tthaixachnhan='Y' ">
            ...
            <p  style="line-height: 1.5; margin-top: 1pt; direction: ltr; padding-bottom: 0pt "></p>
            <p  style="margin-left: 62px; font-size:15px;"> Tên ngân hàng: &#160;
			   <span >
