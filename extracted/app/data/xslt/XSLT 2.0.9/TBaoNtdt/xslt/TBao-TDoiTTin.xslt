...
<xsl:include href="../common/common.xsl"/>
	<xsl:template match="/">
		...
		<div>
			...
			<br/>
			Ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'yyyy')"/>,  Cơ quan Thuế đã thay đổi thông tin tài khoản của người nộp thuế như sau:<br/>
		...
		<table border="1" cellpadding="2" cellspacing="0">
				 ...
				 <xsl:for-each select="$ndungTBao/DSachTKhaiDKy/SacThue">
					 <tr><td colspan="5"><b><xsl:value-of select="tenSacThue"/> </b></td></tr>
				      <xsl:for-each select="DangKyToKhai/ToKhai">
				           ...
				           <tr>
							  <td align="center"><xsl:value-of select="$currentRows" /></td>
			...
			Để sử dụng dịch vụ đơn vị truy cập theo đường dẫn: http://kekhaithue.gdt.gov.vn<br/><br/>
