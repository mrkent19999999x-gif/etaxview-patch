 ...
 <xsl:include href="../common/common.xsl"/>
  <xsl:template match="/">
    <style type="text/css">ol { margin: 0; padding: 0 }
      ...
      .c14 {  border-style: solid; border-color: #000000; border-width: 0.5pt; padding: 2px; }
	...
	<xsl:variable name="NGAY_THONGBAO_NEW"   select="$tbThue1/NGAY_THONGBAO" />
	  ...
	  <xsl:variable name="date" select="10000 * substring($NGAY_THONGBAO_NEW, 7, 4) + 100 * substring($NGAY_THONGBAO_NEW, 4, 2) + substring($NGAY_THONGBAO_NEW, 1, 2)"/>
        ...
        <xsl:variable name="TEN_NTHAY" select="$NDUNG_TBAO/THONGBAO_HDR/TEN_NTHAY" />
        <xsl:variable name="MST_NTHAY" select="$NDUNG_TBAO/THONGBAO_HDR/MST_NTHAY" />
          ...
          <div id="headermct">
			...
			ngày <xsl:value-of select="ihtkk:stringDatetime($NGAY_THONGBAO,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($NGAY_THONGBAO,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($NGAY_THONGBAO,'yyyy')"/>,
              <span class="c1">Ngân hàng
				...
				thông báo trạng thái thực hiện thủ tục nộp thuế điện tử của người nộp thuế, cụ thể như sau:  </span>
            </p>
          </div>
        </xsl:for-each>
        <div id="conten">
          ...
          <table cellpadding="0" cellspacing="0" class="table_hd">
              ...
              <xsl:for-each select="$NDUNG_TBAO/THONGBAO_CTIET/ROW_CTIET">
                <tr>
                 <td style="text-align: center !important;  ">
                  1
                                  ...
                                  <td class="c14" style="text-align: left !important; ">
                 Số GNT
                </td>
                  <td style=" padding-left:5px; height:23px;text-align: left;">
                ...
                <tr>
                 ...
                 2
                  ...
                  <td style=" padding-left:5px; height:23px;text-align: left;"><span class="contenms9">
                 ...
                 3
                  ...
                  <td class="c14" style=" padding-left:5px; text-align: left !important">
                  MST thụ hưởng
                </td>
               <td style=" padding-left:5px; height:23px;text-align: left;">
                    <span class="contenms9"> 				
           ...
           <tr>
                  ...
                  <td style=" padding-left:5px; height:23px;text-align: left;">
                    <span class="contenms9" id="tongtien">
                      <xsl:if test="TONGTIENNOP !=''">
                         <span class="tiennop" id="tongtien"> <xsl:value-of select="ihtkk:formatNumber(TONGTIENNOP)"/></span>							
                      </xsl:if>
                    </span>
                  </td>
                  </tr>
                 ...
                 <td class="c14" style=" padding-left:5px; text-align: left !important">
				  Trạng thái
					 ...
					 <span class="contenms19">
					   <xsl:if test="MA_TTHAI ='11'">
                       Nộp thuế thành công
                      </xsl:if>
                      <xsl:if test="MA_TTHAI ='04'">
                       Lỗi chữ ký số NNT
                      </xsl:if>
					     <xsl:if test="MA_TTHAI ='05'">
                       Lỗi chữ ký số 
                      </xsl:if>
                       ...
                       Lỗi chữ ký số 
					       ...
					       <xsl:if test="MA_TTHAI ='06'">
                       Lỗi đăng ký sử dụng dịch vụ
                      </xsl:if>
					       <xsl:if test="MA_TTHAI ='07'">
                      Lỗi sai cấu trúc chứng từ
                      </xsl:if>
					       <xsl:if test="MA_TTHAI ='08'">
                         Giao dịch không thành công 
                      </xsl:if>
					       <xsl:if test="MA_TTHAI ='09'">
                    Giao dịch không thành công 
                      </xsl:if>
					       <xsl:if test="MA_TTHAI ='10'">
                     Lỗi chữ ký số NHTM
                      </xsl:if>
					       <xsl:if test="MA_TTHAI ='12'">
            ...
            <xsl:if test="MA_TTHAI !='11'">
             ...
             <tr>
                          <td style="width: 27.7pt; border-style: solid; border-color: #000000; border-width: 0.5pt; padding: 2px; text-align: center !important">
                 13
                </td>
                  <td class="c14" style="  padding-left:5px; text-align: left !important">
                  Lý do
	...
	<!--The end -->
        ...
        <div id="conten">
          ...
          <p class="c3">
            <span class="c1 ">Để tra cứu thông tin đã nộp nói trên, xin vui lòng truy cập theo đường dẫn:&#160;</span>
            <span class="c1 c23">
              <a href="htt://nopthue.gdt.gov.vn">http://nopthue.gdt.gov.vn</a> hoặc <a href="http://thuedientu.gdt.gov.vn">http://thuedientu.gdt.gov.vn</a>
            </span>
          </p>
          </div>
        </div>
