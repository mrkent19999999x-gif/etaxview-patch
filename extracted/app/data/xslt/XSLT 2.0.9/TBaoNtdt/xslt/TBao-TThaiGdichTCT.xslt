  ...
  <xsl:variable name="date" select="10000 * substring($NGAY_THONGBAO_NEW, 7, 4) + 100 * substring($NGAY_THONGBAO_NEW, 4, 2) + substring($NGAY_THONGBAO_NEW, 1, 2)"/>
    ...
    <xsl:choose>
		...
		<xsl:call-template name="tbaoHeader1">
			...
			<xsl:with-param name="motaTBao"   select="'V/v Xác nhận chứng từ nộp thuế điện tử'" />
		</xsl:call-template>
			...
			<br/>
				...
				<xsl:if test="$tbThue/TEN_NNOP!=''">		
					...
					<br/>
					<br/>
						 <i><span>&#160;&#160;&#160;	&#160;&#160;Mã số thuế của NNT:&#160; </span></i>																	 
					<xsl:choose>
		...
		&#160;&#160;&#160;&#160;&#160;Căn cứ chứng từ nộp thuế điện tử của người nộp thuế gửi tới cơ quan thuế
			...
			ngày <xsl:value-of select="ihtkk:stringDatetime($tbThue/NGAY_HOSO,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($tbThue/NGAY_HOSO,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($tbThue/NGAY_HOSO,'yyyy')"/>, Cơ quan thuế xác nhận việc nộp chứng từ nộp thuế điện tử của người nộp thuế, cụ thể như sau: <br/>
			<br/>
       ...
       <br/>
		...
		<br/>
		Để tra cứu thông tin đã nộp nói trên, xin vui lòng truy cập theo đường dẫn: http://thuedientu.gdt.gov.vn hoặc http://nopthue.gdt.gov.vn.
		<br/>
