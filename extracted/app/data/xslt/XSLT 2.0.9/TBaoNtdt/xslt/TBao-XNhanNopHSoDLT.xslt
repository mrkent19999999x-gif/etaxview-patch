...
<xsl:include href="../common/common.xsl"/>
	<xsl:template match="/">
		...
		<div>
			...
			, ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayXacNhan,'yyyy')"/>, Cơ quan Thuế đã nhận được hồ sơ khai thuế của đơn vị, gồm có:<br/>
			<table>
				<tbody>
					...
					<tr>
						...
						<td><xsl:value-of select="$ndungTBao/DSachHSoNop/NDungDSach/tenTKhai-phuluc"/></td>
						<td><xsl:value-of select="$ndungTBao/DSachHSoNop/NDungDSach/loaiTKhai"/></td>
