					...
					<div id="contentprint">
						<xsl:for-each select="$NDUNG_TBAO">
							<div id="headermct">
								...
								<p class="c3 c11">
									<span class="c1 c0 c4">
										<xsl:if test="$THONGBAO_HDR/TEN_TBAO !=''">
											<span>
												<xsl:value-of select="$THONGBAO_HDR/TEN_TBAO" />
											</span>
										</xsl:if>
								...
								lập và nộp hồ sơ khai thuế kỳ này đúng thời hạn quy định để đảm
								bảo quyền lợi theo pháp luật của người nộp thuế. Các hồ sơ khai
								thuế phải nộp kỳ này của người nộp thuế như sau:
							</div>
						</xsl:for-each>
						<div id="conten">
							<p class="c2 c11">&#160;</p>
