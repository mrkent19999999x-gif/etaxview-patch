...
<xsl:include href="../common/common.xsl"/>
	<xsl:template match="/">
		...
		<xsl:call-template name="tbaoHeader">
			<xsl:with-param name="mauTBao"   select="'02/TB-KĐT'"/>
		...
		<div>
			...
			<p><xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayCapTK,'hh')"/> giờ <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayCapTK,'mi')"/> phút <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayCapTK,'ii')"/> giây, ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayCapTK,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayCapTK,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayCapTK,'yyyy')"/>, đơn vị đã được Cơ quan Thuế cấp tài khoản sử dụng hình thức khai thuế điện tử với các thông tin tài khoản như sau:</p>
			<p>
