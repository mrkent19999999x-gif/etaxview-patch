...
<xsl:include href="../common/common.xsl"/>
	<xsl:template match="/">
		...
		<xsl:call-template name="hsoHeaderTVan">
			<xsl:with-param name="mauTBao"   select="'05-ĐK/T-VAN'"/>
		...
		<div>
			<p>Cơ sở chúng tôi xin đăng ký các loại hồ sơ khai thuế nộp qua tổ chức cung cấp dịch vụ giá trị gia tăng về giao dịch điện tử trong lĩnh vực thuế (T-VAN).</p>
			<b>1. Tổ chức cung cấp dịch vụ T-VAN: <xsl:value-of select="$nDungDKy/ten_T_VAN"/></b><br/>
			<b>2. Danh sách hồ sơ khai thuế đăng ký nộp qua dịch vụ T-VAN:</b><br/>
			<table width="100%" cellpadding="3" cellspacing="2" style="border-collapse: collapse; text-align: center;">
					<tr bgcolor="#f0f0f0" style="font-weight: bold;">
						...
						<th style="border: 1px solid #000;">Kỳ tính thuế bắt đầu nộp</th>
					</tr>
					<xsl:for-each select="$nDungDKy/DSachHSoDKyNop/NoiDungChiTiet">
					<tr>
						...
						<td style="border: 1px solid #000;"><xsl:value-of select="kyBDau"/></td>
					</tr>		
					</xsl:for-each>		
				</table>
				<p>Chúng tôi xin cam kết tính thuế, khai thuế và nộp thuế theo đúng các quy định pháp luật về thuế ./.</p>
			<hr/>
