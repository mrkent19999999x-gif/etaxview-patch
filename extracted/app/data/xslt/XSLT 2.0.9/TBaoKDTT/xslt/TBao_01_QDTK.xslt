...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		<xsl:call-template name="tbaoHeader">
			<xsl:with-param name="mauTBao"   select="'01/QĐ-TK'"/>
			...
			<xsl:with-param name="motaTBao_1"   select="'bằng biện pháp trích tiền từ tài khoản của đối tượng bị cưỡng chế'" />
			<xsl:with-param name="motaTBao_2"   select="'tại kho bạc nhà nước, tổ chức tín dụng'" />
			...
			<xsl:with-param name="diaDanh"   select="$ndungTBao/diaDanh" />
					...
					<br/>
							...
							<p class="align-j fz18_5px">
								&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;<b>Điều 1.</b> Cưỡng chế bằng biện pháp trích tiền từ tài khoản của đối tượng bị cưỡng chế tại Kho bạc nhà nước/Ngân hàng thương mại/Tổ chức tín dụng để thi hành quyết định hành chính thuế số
								...
								<div>
								&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Yêu cầu Kho bạc nhà nước/Ngân hàng thương mại/Tổ chức tín dụng phong tỏa tài khoản
								<xsl:if test="$ndungTBao/QuyetDinh/Dieu1/d11TT215TKBiPToa !=''">
									<span>
										<xsl:value-of select="$ndungTBao/QuyetDinh/Dieu1/d11TT215TKBiPToa" />
									</span>
								</xsl:if>
								(đối với trường hợp thực hiện theo qui định tại điểm d Khoản 3 Điều 11 Thông tư số 215/2013/TT-BTC);
								</div>
							...
							<p class="align-j fz18_5px">
								&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Kho bạc nhà nước/Ngân hàng thương mại/Tổ chức tín dụng
								<xsl:if test="$ndungTBao/QuyetDinh/Dieu2/ttinToChucCoTrachNhiemTrichNo !=''">
									<span>
										<xsl:value-of select="$ndungTBao/QuyetDinh/Dieu2/ttinToChucCoTrachNhiemTrichNo" />
									</span>
								</xsl:if>
								có trách nhiệm trích số tiền nêu tại Điều 1 của Quyết định này để nộp vào tài khoản số
								<xsl:if test="$ndungTBao/QuyetDinh/Dieu2/trichTienVaoTKSo !=''">
									<span>
										<xsl:value-of select="$ndungTBao/QuyetDinh/Dieu2/trichTienVaoTKSo" />
									</span>
								</xsl:if>
								mở tại kho bạc
								<xsl:if test="$ndungTBao/QuyetDinh/Dieu2/trichTienVaoKBNN !=''">
									<span>
										<xsl:value-of select="$ndungTBao/QuyetDinh/Dieu2/trichTienVaoKBNN" />
									</span>
								</xsl:if>
								.Trường hợp trên tài khoản của ông (bà)/tổ chức
								<xsl:if test="$ndungTBao/QuyetDinh/Dieu2/ttinDTuongNChinhTHienQD !=''">
									<span>
										<xsl:value-of select="$ndungTBao/QuyetDinh/Dieu2/ttinDTuongNChinhTHienQD" />
									</span>
								</xsl:if>
								nhỏ hơn số tiền trên Quyết định cưỡng chế nợ thì Kho bạc nhà nước/Ngân hàng thương mại/Tổ chức tín dụng
								<xsl:if test="$ndungTBao/QuyetDinh/Dieu2/ttinToChucCoTrachNhiemTrichNo !=''">
									<span>
										<xsl:value-of select="$ndungTBao/QuyetDinh/Dieu2/ttinToChucCoTrachNhiemTrichNo" />
									</span>
								</xsl:if>
								vẫn phải trích số tiền còn lại sau khi trừ đi số dư tối thiểu để duy trì tài khoản và tiếp tục theo dõi, trích tiếp số tiền phát sinh có trên tài khoản của đơn vị trong thời gian quyết định có hiệu lực.
							</p>
							<div>
