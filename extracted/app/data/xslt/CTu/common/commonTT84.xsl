...
<!--  Ký tự : && trong đây được viết bằng : &#38;&#38;  (dpt)-->

<msxsl:script language="JavaScript" implements-prefix="ihtkk">
var flag = true;
var arr1=new Array(""," nghìn"," triệu"," tỷ")
var arr2=new Array("không","mười","hai mươi","ba mươi","bốn mươi","năm mươi","sáu mươi","bảy mươi","tám mươi","chín mươi")
...
function dochangtrieu(so,daydu)
 ...
 function dochangchuc(so,daydu)
  ...
  // format tong tien  usd
function toFormatDecimal1(inpNumber) {
	var pnumber = "";	
    var result = 0;	
	pnumber = inpNumber;
    var ktusauTphan = pnumber.lastIndexOf(".");
    var chuoi = pnumber.split(".");
    var gtri = chuoi[1];
	     var num = formatNumberUSD1(chuoi[0]);
    if (ktusauTphan ==  - 1) {
        result = num;
    }
    else {
        ...
        if (gtri.length > 2) {
           ...
           result = result.replace(".",",");
            ...
            var kq_c= result.split(".");
            result = formatNumberUSD1(kq_c[0])+","+kq_c[1];
        }
   ...
   function formatNumberUSD1(vNumber){
	 var strReturn = "";
  ...
  // format tong tien  usd
function toFormatDecimal(inpNumber) {
	var pnumber = "";	
    var result = 0;
	var node = inpNumber.nextNode();	
	pnumber = node.childNodes[0].nodeValue;

    var ktusauTphan = pnumber.lastIndexOf(".");
    var chuoi = pnumber.split(".");
    var gtri = chuoi[1];
	     var num = formatNumberUSD(chuoi[0]);
    if (ktusauTphan ==  - 1) {
        result = num;
    }
    else {
        ...
        if (gtri.length > 2) {
           ...
           result = result.replace(",",".");
            result = financial(result);
            var kq_c= result.split(".");
            result = formatNumberUSD(kq_c[0])+","+kq_c[1];
        }
...
function toFormatDecimall(inpNumber) {
	var pnumber = "";	
    var result = 0;
	var node = inpNumber.nextNode();	
	pnumber = node.childNodes[0].nodeValue;
   ...
   function formatNumberUSD(vNumber){
	var strReturn = "";
	var firstChar = "";
	var fnum = 0;
	var fNumTemp = "";
