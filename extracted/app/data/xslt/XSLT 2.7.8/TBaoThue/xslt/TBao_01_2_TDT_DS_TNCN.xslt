...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ihtkk="http://www.nhantokhai.gdt.gov.vn/xslt">
	...
	<xsl:template match="/">
		...
		<xsl:call-template name="tbaoHeader_01_2TDT_19_DS_TNCN">
		...
		<xsl:if test="$ndungTBao/trangThai='N'">
			<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Căn cứ Danh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân do  <xsl:value-of select="$ndungTBao/tenNNT" />&#160;(MST: <xsl:value-of select="$ndungTBao/mst"/>)
				...
				<xsl:value-of select="$ndungTBao/maGiaoDichDTu"/>. Cơ quan thuế thông báo về việc không chấp nhận Danh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân của
			...
			<br/>
			<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160; Đề nghị <xsl:value-of select="$ndungTBao/tenNNT" /> điều chỉnh và gửi lại Danh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân theo hướng dẫn nêu trên.
</div>
		</xsl:if>
		<xsl:if test="$ndungTBao/trangThai='Y'and $ndungTBao/DulieuPsinhLoi/CTietDuLieuPSinh/dLieuLoi = ''">
			<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Căn cứ Danh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân do
				<xsl:value-of select="$ndungTBao/tenNNT"/>
				...
				<xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoSo,'mm')"/>
				năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoSo,'yyyy')"/>, mã giao dịch điện tử:
				<xsl:value-of select="$ndungTBao/maGiaoDichDTu"/>. Cơ quan thuế thông báo về việc chấp nhậnDanh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân của
				<xsl:value-of select="$ndungTBao/tenNNT"/>
				<i> gửi tới cơ quan thuế</i>, cụ thể như sau:
			</div>
			<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160; - Ngày gửi:
			...
			<br/>
			<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160; “Danh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân” do người nộp thuế gửi được cơ quan thuế chấp nhận vào lúc
				<xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoSo,'hh')"/>
				giờ
				<xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoSo,'mi')"/>
				phút ngày
			...
			<br/>
			<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160; Cơ quan thuế sẽ thực hiện trả kết quả chấp nhận “Danh sách chi tiết số tiền nộp thuế TNCN đã nộp thay cho từng cá nhân” cho người nộp thuế qua Cổng thông tin điện tử của CỤC THUẾ.</div>
			<br/>
		</xsl:if>
		<!--<xsl:if test="$ndungTBao/trangThai='Y' and $ndungTBao/DulieuPsinhLoi/CTietDuLieuPSinh/dLieuLoi != ''">
