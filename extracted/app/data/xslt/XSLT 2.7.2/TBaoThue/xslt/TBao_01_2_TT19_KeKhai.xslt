...
<xsl:include href="../common/common.xsl"/>
	<xsl:template match="/">
		...
		<xsl:call-template name="tbaoHeader_02_TDKTT">
		...
		<br/>
				...
				<xsl:if test="$ndungTBao/trangThai='N'">
						<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Căn cứ Văn bản đề nghị thay đổi kỳ tính thuế từ tháng sang quý mẫu số 01/ĐK-TĐKTT của <xsl:value-of select="$NNhanTBaoThue/tenNNhan"/>
						<xsl:if test="$ttinDaiLyThue/tenDLT !=''">
						do <xsl:value-of select="$ttinDaiLyThue/tenDLT"/> 
						</xsl:if>
						gửi tới cơ quan thuế <xsl:value-of select="$ttinCQT/tenCQT"/> đã được cổng thông tin điện tử của CỤC THUẾ thông báo tiếp nhận vào lúc <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'hh')"/> giờ <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'mi')"/> phút ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'yyyy')"/>
						mã giao dịch điện tử <xsl:value-of select="$ndungTBao/maGiaoDichDTu"/>. Cơ quan thuế thông báo về việc không chấp nhận Văn bản đề nghị thay đổi kỳ tính thuế từ tháng sang quý mẫu số 01/ĐK-TĐKTT của <xsl:value-of select="$NNhanTBaoThue/tenNNhan"/> 
						<xsl:if test="$ttinDaiLyThue/tenDLT !=''">
						do <xsl:value-of select="$ttinDaiLyThue/tenDLT"/> 
						</xsl:if>
						gửi tới cơ quan thuế <xsl:value-of select="$ttinCQT/tenCQT"/> , cụ thể như sau:</div>
						<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;- Tên tờ khai: <xsl:value-of select="$ndungTBao/HoSoThue/CTietHoSoThue/tokhai-phuluc"/></div>
						...
						<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;- Loại thuế: <xsl:value-of select="$ndungTBao/HoSoThue/CTietHoSoThue/loaiThue"/></div>
						...
						<!-- <div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Đề nghị <xsl:value-of select="$NNhanTBaoThue/tenNNhan"/> điều chỉnh và gửi lại Văn bản đề nghị thay đổi kỳ tính thuế từ tháng sang quý mẫu số 01/ĐK-TĐKTT theo hướng dẫn nêu trên để được ghi nhận nghĩa vụ nộp hồ sơ thuế/giải quyết hồ sơ thuế theo quy định (trừ hồ sơ đăng ký thuế).</div>
				 --></xsl:if>	
				<xsl:if test="$ndungTBao/trangThai='Y'">
						...
						gửi tới cơ quan thuế <xsl:value-of select="$ttinCQT/tenCQT"/> đã được cổng thông tin điện tử của CỤC THUẾ thông báo tiếp nhận vào lúc <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'hh')"/> giờ <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'mi')"/> phút ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayNopThucTe,'yyyy')"/>
						mã giao dịch điện tử <xsl:value-of select="$ndungTBao/maGiaoDichDTu"/>. Cơ quan thuế thông báo về việc chấp nhận Văn bản đề nghị thay đổi kỳ tính thuế từ tháng sang quý mẫu số 01/ĐK-TĐKTT của <xsl:value-of select="$NNhanTBaoThue/tenNNhan"/> 
						<xsl:if test="$ttinDaiLyThue/tenDLT !=''">
						do <xsl:value-of select="$ttinDaiLyThue/tenDLT"/> 
						</xsl:if>
						gửi tới cơ quan thuế <xsl:value-of select="$ttinCQT/tenCQT"/> , cụ thể như sau:</div>
								...
								<tr>
								 ...
								 <xsl:for-each select="$ndungTBao/IDKhoanNop/CTietIDKhoanNop">
											...
											<tr>
											  ...
											  <td class="align-c">
											    <xsl:if test="$ndungTBao/HoSoThue/CTietHoSoThue/loaiToKhai !='Chính thức'">
						...
						<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Văn bản đề nghị thay đổi kỳ tính thuế từ tháng sang quý mẫu số 01/ĐK-TĐKTT của người nộp thuế được cơ quan thuế chấp nhận vào lúc <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoanThanh,'hh')"/> giờ <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoanThanh,'mi')"/> phút ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoanThanh,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoanThanh,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayHoanThanh,'yyyy')"/>. 
						Ngày hoàn thành việc nộp HSKT điện tử của NNT là ngày <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayChapNhan,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayChapNhan,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($ndungTBao/ngayChapNhan,'yyyy')"/>.</div><br/>
		...
		<div>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;Trường hợp cần biết thêm thông tin chi tiết, Người nộp thuế vui lòng liên hệ với Cơ quan thuế quản lý trực tiếp để được hỗ trợ.</div> 
