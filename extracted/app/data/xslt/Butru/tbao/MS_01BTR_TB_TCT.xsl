...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<xsl:for-each select="THONGBAO">
			...
			<xsl:variable name="LOAI_DN" select="./NDUNG_TBAO/LOAI_DN" />
			...
			<div style="margin: auto; position: relative; width: 100%;">
				...
				<p
					...
					<span
						...
						Về việc tiếp nhận hồ sơ đề nghị bù trừ khoản nộp thừa
					</span>
				</p>
				<p
				...
				<xsl:if test="$TTHAI_XNHAN ='Y'">
					<p
						...
						&#160;&#160;&#160;&#160;Hồ sơ đề nghị bù trừ khoản nộp thừa của người nộp thuế đã được cơ quan thuế tiếp nhận vào lúc
						<xsl:if test="$NGAY_THONGBAO !=''">
							<xsl:value-of
								select="java:com.seatech.etax.common.utils.DateTimeUtil.getFullDateString(string($NGAY_THONGBAO),'dd/MM/yyyy HH:mm:ss','F1')" />.
						</xsl:if>
					...
					<p
						style="line-height: 1.5; padding-top: 6pt; direction: ltr; padding-bottom: 0pt;font-size: 15px; font-family: Times New Roman; ">
						&#160;&#160;&#160;&#160;Hồ sơ đề nghị bù trừ khoản nộp thừa sẽ được cơ quan thuế tiếp tục kiểm tra và trả Thông báo chấp nhận hoặc không chấp nhận trong thời gian 01 (một) ngày làm việc kể từ ngày cơ quan thuế tiếp nhận hồ sơ của người nộp thuế ghi trên Thông báo này.
					</p>
				</xsl:if>
			</div>
