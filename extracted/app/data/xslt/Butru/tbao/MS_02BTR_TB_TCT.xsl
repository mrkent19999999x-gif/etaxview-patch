	...
	<xsl:template match="/">
		<xsl:for-each select="THONGBAO">
			...
			<div style="margin: auto; position: relative; width: 100%;">
							...
							<td
								...
								<span
									...
									<xsl:if test="$NGAY_THONGBAO !=''">
										<xsl:value-of
											select="java:com.seatech.etax.common.utils.DateTimeUtil.getFullDateString(string($NGAY_THONGBAO),'dd/MM/yyyy HH:mm:ss','F2')" />
									</xsl:if>
								</span>
							</td>
						</tr>
				...
				<xsl:if test="$TTHAI_XNHAN ='Y'">
					<p style="line-height: 1.5; padding-top: 6pt; direction: ltr; padding-bottom: 0pt;font-size: 15px; font-family: Times New Roman; ">
						&#160;&#160;&#160;&#160;Căn cứ hồ sơ đề nghị bù trừ khoản nộp thừa của
						...
						<xsl:if test="$NGAY_THONGBAO !=''">
							<xsl:value-of
								select="java:com.seatech.etax.common.utils.DateTimeUtil.getFullDateString(string($NGAY_THONGBAO),'dd/MM/yyyy HH:mm:ss','F1')" />
						</xsl:if>, mã giao dịch điện tử
						<xsl:if test="$MA_THAMCHIEU !=''">
							<xsl:value-of select="$MA_THAMCHIEU" />
						</xsl:if>.
						Cơ quan thuế thông báo về việc chấp nhận hồ sơ đề nghị bù trừ khoản nộp thừa của
						<xsl:if test="$TEN_NNOP !=''">
							<xsl:value-of select="$TEN_NNOP" />
						</xsl:if>
						<xsl:if test="$MST_DLT !=''">
					...
					<p
						...
						<xsl:if test="$LOAI_VB !='03'">
							ngày <xsl:value-of select="$NGAY_TRA_KQUA" /> kể từ ngày tiếp nhận hồ sơ đề nghị bù trừ khoản nộp thừa.
						</xsl:if>
						<xsl:if test="$LOAI_VB ='03'">
					...
					<p
						style="line-height: 1.5; padding-top: 6pt; direction: ltr; padding-bottom: 0pt;font-size: 15px; font-family: Times New Roman; ">
						&#160;&#160;&#160;&#160;Cơ quan thuế sẽ thực hiện trả kết quả giải quyết hồ sơ đề nghị bù trừ khoản nộp thừa cho người nộp thuế qua Hệ thống Thuế điện tử eTax (<b><a style="color:#666;" href="http://thuedientu.gdt.gov.vn">http://thuedientu.gdt.gov.vn</a></b>).
					</p>
				</xsl:if>
				<xsl:if test="$TTHAI_XNHAN ='N'">
					<p style="line-height: 1.5; padding-top: 6pt; direction: ltr; padding-bottom: 0pt;font-size: 15px; font-family: Times New Roman; ">
						...
						Cơ quan thuế thông báo về việc không chấp nhận hồ sơ đề nghị bù trừ khoản nộp thừa của
						<xsl:if test="$TEN_NNOP !=''">
							<xsl:value-of select="$TEN_NNOP" />
						</xsl:if>
						<xsl:if test="$MST_DLT !=''">
			...
			<div style="float: left; margin: auto; position: relative; width: 100%;">
				...
				<p style="line-height: 1.5; padding-top: 6pt; direction: ltr; padding-bottom: 0pt">
					<span style="font-size: 15px; font-family: Times New Roman; line-height: 1.5; padding-top: 6pt; height: 11pt; direction: ltr; padding-bottom: 0pt">
						&#160;&#160;&#160;&#160;Nếu có vướng mắc, xin vui lòng truy cập theo đường dẫn <b><a style="color:#666;" href="http://thuedientu.gdt.gov.vn">http://thuedientu.gdt.gov.vn</a></b>
						hoặc liên hệ với
						<xsl:if test="$TEN_CQT !=''">
							<xsl:value-of select="$TEN_CQT" />
