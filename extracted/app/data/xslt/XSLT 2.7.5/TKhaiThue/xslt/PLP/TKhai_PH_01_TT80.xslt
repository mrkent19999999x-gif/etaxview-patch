...
'" />
		...
		<xsl:with-param name="mauTKhai"   select="'01/PH'"/>
							 ...
							 <xsl:for-each select="$tkchinh/NoiDung01PH/ChiTietPhi">
							  ...
							  <tr>
							   ...
							   <td class="align-r"><b><xsl:value-of select="ihtkk:formatNumber(tongCong)"/></b></td>
								...
								<tr>
								<xsl:if test="$tkchinh/NoiDung01PH_TT63/ChiTietPhiTT63/ten_LoaiPhi_TT63!= ''" >
							  <td colspan="8"><b>Phí thu theo các Thông tư hỗ trợ tháo gỡ khó khăn</b>
							  </td>
							  </xsl:if>
								</tr>
								<xsl:for-each select="$tkchinh/NoiDung01PH_TT63/ChiTietPhiTT63">
									<xsl:variable name="currentRows" select='position()' />
								<tr>	
							  <xsl:if test="ten_LoaiPhi_TT63!= ''" >
							   <td class="align-c"><xsl:value-of select="$currentRows" /></td>
							   <td class="align-l"><xsl:value-of select="ten_LoaiPhi_TT63"/></td>
							   <td class="align-c"><xsl:value-of select="tieuMuc_TT63"/></td>
							   <td class="align-c"><xsl:value-of select="ma_DonViTien_TT63"/></td>
							   <td class="align-r"><xsl:value-of select="ihtkk:formatNumber(soTienPHThuDuoc_TT63)"/></td>
							   <td></td>
							   <td></td>
							   <td></td>
							   </xsl:if>
