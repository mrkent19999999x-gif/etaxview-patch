 ...
 <xsl:include href="../../include/TKhaiFooter.xsl"/>    	
	...
	<xsl:template match="/">
		...
		<xsl:call-template name="tkhaiHeaderDKThue_HS">
			<xsl:with-param name="mauTKhai" select="'05/ĐK-T-VAN'"/>
		</xsl:call-template>
	...
	<div style="padding-left:100px;"> Mã số thuế: <xsl:value-of select="$ttchungdk/TTinDKyThue/tIN"/></div><br/>
		<div>Cơ sở chúng tôi xin đăng ký các loại hồ sơ khai thuế nộp qua tổ chức cung cấp dịch vụ giá trị gia tăng về giao dịch điện tử trong lĩnh vực thuế (T-VAN).</div><br/>
			...
			<div class="content">            
				 ...
				 <table class="tkhai_table">
                        <tr>
							...
							<td class="align-left"><b>Kỳ tính thuế bắt đầu nộp</b></td>
						</tr>
                        <xsl:for-each select="$ndungdk/DSachHSoDKyNop/NoiDungChiTiet">
						    <tr>
								        <td><xsl:value-of select="maHSo"/></td>
										<td><xsl:value-of select="tenHSo"/></td>
										<td><xsl:value-of select="kyBDau"/></td>
							</tr>
						    </xsl:for-each>
						    </table><br/>
				 <div>Chúng tôi xin cam kết tính thuế, khai thuế và nộp thuế theo đúng các quy định pháp luật về thuế ./.</div>
