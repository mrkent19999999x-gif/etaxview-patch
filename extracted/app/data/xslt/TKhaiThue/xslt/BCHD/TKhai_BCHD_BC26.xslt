 ...
 <xsl:include href="../../common/common.xsl"/> 
	<xsl:template match="/">
		...
		<div class="ndungtkhai_div">
	        ...
	        <table>
				<tr>
					<td>Kỳ báo cáo cuối cùng [<xsl:if test="$tkchinh/kyBCaoCuoi = 'true' "> x </xsl:if> <xsl:if test="$tkchinh/kyBCaoCuoi = '1' "> x </xsl:if>]	</td>
					<td>Chuyển địa điểm [<xsl:if test="$tkchinh/chuyenDiaDiem = 'true' "> x </xsl:if> <xsl:if test="$tkchinh/chuyenDiaDiem = '1' "> x </xsl:if>]	</td>
				</tr>
				<tr>
            ...
            <table class="tkhai_table">
                        <tr>
							...
							<td class="align-c" rowspan="4"><b>Tên loại hóa đơn</b></td>
							<td class="align-c" rowspan="4"><b>Ký hiệu mẫu hóa đơn</b></td>
							<td class="align-c" rowspan="4"><b>Ký hiệu hóa đơn</b></td>
							<td class="align-c" colspan="5"><b>Số tồn đầu kỳ, mua/phát trong kỳ</b></td>
                        ...
                        <xsl:for-each select="$tkchinh/HoaDon/ChiTiet">
							...
							<tr>
								...
								<td><xsl:value-of select="tenHDon"/></td>
								...
								<td class="align-r"><xsl:value-of select="ihtkk:breakWord(soDaSDung)"/></td>
								<td class="align-r"><xsl:value-of select="ihtkk:formatNumber(xoaBo_soLuong)"/></td>
								<td class="align-c"><xsl:value-of select="ihtkk:breakWord(xoaBo_so)"/></td>
								<td class="align-r"><xsl:value-of select="ihtkk:formatNumber(mat_soLuong)"/></td>
								<td class="align-c"><xsl:value-of select="ihtkk:breakWord(mat_so)"/></td>
								<td class="align-r"><xsl:value-of select="ihtkk:formatNumber(huy_soLuong)"/></td>
								<td class="align-c"><xsl:value-of select="ihtkk:breakWord(huy_so)"/></td>
