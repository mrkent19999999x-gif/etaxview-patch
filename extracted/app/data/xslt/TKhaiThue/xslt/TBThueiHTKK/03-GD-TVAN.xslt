...
<xsl:include href="../../include/TKhaiHeader.xsl"/>
 <xsl:include href="../../common/common.xsl"/> 
	<xsl:template match="/">
	    ...
	    <xsl:with-param name="mauTKhai" select="'03-GD/T-VAN'"/>
	  </xsl:call-template>
     ...
     <div>Chi tiết kết quả giao dịch:</div>
                ...
                <table class="tkhai_table">
                        <tr>
							...
							<td class="align-left"><b>Kỳ bắt đầu</b></td>
                        ...
                        <xsl:for-each select="$noidungtb/CTietGDich/NoiDung03GDTVAN">
					    <xsl:variable name="currentRows" select="position()"/>
						    <tr>
										<td class="align-c"><xsl:value-of select="$currentRows"/></td>
										<td><xsl:value-of select="maHSo"/></td>
										<td><xsl:value-of select="tenHSo"/></td>
											<td class="align-left"><xsl:value-of select="kyBDau"/></td>
