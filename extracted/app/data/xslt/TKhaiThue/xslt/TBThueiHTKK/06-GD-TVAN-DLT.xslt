...
<xsl:include href="../../include/TKhaiHeader.xsl"/>
 <xsl:include href="../../common/common.xsl"/> 
	<xsl:template match="/">
	    ...
	    <xsl:with-param name="mauTKhai" select="'06-GD/T-VAN-ĐLT'"/>
	  </xsl:call-template>
    ...
    <div>Mã giao dịch: <xsl:value-of select="$noidungtb/GDich/maGDich"/></div>
     ...
     <div style="padding-left:70px">Mã số thuế: <xsl:value-of select="$noidungtb/CTietGDich/tinCTietGDich"/></div>
      <div style="padding-left:70px">Tên đại lý thuế: <xsl:value-of select="$noidungtb/CTietGDich/tenNNT"/></div>
     <div style="padding-left:70px">Ngày ngừng: <xsl:value-of select="$noidungtb/CTietGDich/ngayNgungCTietGDich"/></div>
