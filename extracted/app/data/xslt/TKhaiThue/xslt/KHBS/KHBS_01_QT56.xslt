...
<!--start phu luc 01/KHBS-->
		  ...
		  <xsl:variable name="mauTKhaiChinh"> 
            <xsl:choose>
				...
				<xsl:when test="$maTKhai=12">
                	...
                	<xsl:when test="$maTKhai=03">
                    <xsl:value-of select="'03/TNDN'"/>
                </xsl:when>
                
                <xsl:when test="$maTKhai=08">
                    <xsl:value-of select="'02/TAIN'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=60">
                    <xsl:value-of select="'02/BVMT'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=62">
                    <xsl:value-of select="'02/PHLP'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=74">
                    <xsl:value-of select="'02/NTNN'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=72">
                    <xsl:value-of select="'04/NTNN'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=84">
                    <xsl:value-of select="'03A/TĐ-TAIN'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=88">
                    <xsl:value-of select="'02/TAIN-DK'"/>
                </xsl:when>
                 <xsl:when test="$maTKhai=86">
                    <xsl:value-of select="'02/TNDN-DK'"/>
                </xsl:when>
                <xsl:when test="$maTKhai=304">
                    <xsl:value-of select="'06/TNDN'"/>
                </xsl:when>
		...
		<xsl:with-param name="ghiChuPL"   select="' '"/>
			...
			<div class="content">
                ...
                <div>1. Người nộp thuế tự phát hiện số tiền thuế đã được hoàn phải nộp trả NSNN là <xsl:value-of select="ihtkk:formatNumber($khbs/NOIDUNGGIAITHICH/soTienTraNSNN)"/> đồng thuộc Lệnh hoàn trả khoản thu NSNN hoặc Lệnh hoàn trả kiêm bù trừ khoản thu NSNN số <xsl:value-of select="$khbs/NOIDUNGGIAITHICH/quyetDinhHoanThueSo"/> 
                
                ngày <xsl:value-of select="ihtkk:stringDatetime($khbs/NOIDUNGGIAITHICH/ngay,'dd/mm/yyyy')"/> 
                
                
của <xsl:value-of select="$khbs/NOIDUNGGIAITHICH/tenCQT"/> </div> <br/>
