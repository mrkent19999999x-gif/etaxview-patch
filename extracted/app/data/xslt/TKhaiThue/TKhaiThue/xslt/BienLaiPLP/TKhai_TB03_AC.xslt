 ...
 <xsl:include href="../../common/common.xsl"/> 
	<xsl:template match="/">
		...
		<xsl:variable name="moTaBieuMau" select="'Ban hành kèm theo Thông tư số 303/2016/TT-BTC ngày 15/11/2016 của Bộ Tài chính'" />
		 <xsl:call-template name="tkhaiHeader-TB03-BL">
		...
		<div class="ndungtkhai_div">
            <div class="content">
            <br/> 
            <div>Phương pháp hủy biên lai: <xsl:value-of select="$tkchinh/ppHuyBienLai"/></div><br/>
         
            <div>Hồi <xsl:value-of select="ihtkk:stringDatetime($tkchinh/ngayHuyBienLai,'hh')"/> giờ <xsl:value-of select="ihtkk:stringDatetime($tkchinh/ngayHuyBienLai,'mi')"/>, ngày <xsl:value-of select="ihtkk:stringDatetime($tkchinh/ngayHuyBienLai,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($tkchinh/ngayHuyBienLai,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($tkchinh/ngayHuyBienLai,'yyyy')"/>, <xsl:value-of select="$ttkthue/NNT/tenNNT"/> thông báo hủy biên lai thu tiền phí lệ phí như sau: </div>
            ...
            <table class="tkhai_table">
                        ...
                        <xsl:for-each select="$tkchinh/BKeKQuaHuyBLai/ChiTietBKeKQuaHuyBLai">
...
<tr>
<td>	  	
	    <xsl:call-template name="tkhaiFooter-bc26ac-BLai"/>
<div id="sigDiv"></div>
</td>
</tr>
</table>		
