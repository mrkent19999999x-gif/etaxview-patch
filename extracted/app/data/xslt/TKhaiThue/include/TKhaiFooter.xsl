...
xmlns:ihtkk="http://www.nhantokhai.gdt.gov.vn/xslt" exclude-result-prefixes="xsl xsi ihtkk">
 <xsl:include href="../common/common.xsl"/> 
   ...
   <xsl:template name="tkhaiFooter_DK_TVAN">
	...
	<!--start Footer  TK BK01/AC-->	
	<xsl:template name="tkhaiFooter-bk01ac">
		<xsl:variable name="tkchinh" select='HSoThueDTu/HSoKhaiThue/CTieuTKhaiChinh' />	
	 <div>Cam kết báo cáo tình hình sử dụng hóa đơn trên đây là đúng sự thật, nếu sai, đơn vị chịu hoàn toàn trách nhiệm trước pháp luật.</div>
        <table  width="100%">
            <tr>
                <td class="align-c"><b>Người lập biểu</b></td>
                <td class="align-c"><i>Ngày <xsl:value-of select="ihtkk:stringDatetime($tkchinh/PLuc_3.10/ngayBCao,'dd')"/> tháng <xsl:value-of select="ihtkk:stringDatetime($tkchinh/PLuc_3.10/ngayBCao,'mm')"/> năm <xsl:value-of select="ihtkk:stringDatetime($tkchinh/PLuc_3.10/ngayBCao,'yyyy')"/></i></td>						
            </tr>
			<tr>
				<td class="align-c"><i>(Ký, ghi rõ họ, tên)</i></td>
				<td class="align-c"><b>ĐẠI DIỆN THEO PHÁP LUẬT</b></td>
			...
			<tr>
				<td></td>
				<td class="align-c">
				<xsl:value-of select="$tkchinh/PLuc_3.10/nguoiDaiDien"/></td>
			</tr>             
	    </table>
	</xsl:template>	    
